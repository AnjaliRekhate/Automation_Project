  ...
  stages {
    ...
    stage('Build Docker Image') {
      parallel {
        stage('Build Docker Image') {
          steps {
            sh 'cd vote && sudo docker build . -t 73264ace1394.dkr.ecr.us-east-1.amazonaws.com/vote:${BUILD_NUMBER}'
            sh 'sudo docker push 73264ace1394.dkr.ecr.us-east-1.amazonaws.com/vote:${BUILD_NUMBER}'
    ...
    stage('Deploy in ECS') {
      steps {
        script {
          sh'''
ECR_IMAGE="73264ace1394.dkr.ecr.us-east-1.amazonaws.com/vote:${BUILD_NUMBER}"
...
--task-definition ${TASK_FAMILY}:${NEW_REVISION}'''
  ...
  post {
    always {
      deleteDir()
      sh 'sudo docker rmi 73264ace1394.dkr.ecr.us-east-1.amazonaws.com/vote:${BUILD_NUMBER}'
    }
